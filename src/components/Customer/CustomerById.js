...
export default function CustomerById() {
    useEffect(() => {
        document.title = "Fetch Customer From Id";
    }, []);

    const [customerId, setId] = useState('');
    const fetchCustomer = (e) => {
        console.log(customerId);
        customerFromServer(customerId);
        e.preventDefault();
    }
    ...
    const customerFromServer = (customerId) => {
        //console.log(username);
        axios.get(`${base_url}/api/customers/${customerId}`).then(
            (response) => {
        ...
        <>
            <Form onSubmit={fetchCustomer}>
                <FormGroup className="mx-5">
                    <Label className="my-3">Customer Id</Label>
                    <Input type="number" name="customerId" id="exampleId" placeholder="Enter Customer Id Here" onChange={(event) => {
